#include "LcmSensorDataReceiver.hpp"
...
struct LcmSensorDataReceiver::Helper {
...
addChannel(const std::string& iSensorChannel,
  ...
  case SensorTypePlanarLidar:
    sub = mHelper->mBotWrapper->getLcm()->subscribe
      (iSensorChannel, &LcmSensorDataReceiver::Helper::onLidar, mHelper.get());
    break;
  default:
    return false;
  }
